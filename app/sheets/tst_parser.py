...
def parse_size(text: str) -> dict[str, str]:
    ...
    # if m:
    #     diameter = m.group("diameter").upper().replace("/", "R")
    #     print(diameter)  # → R14
    ...
    result["diam"] = result["diameter"].strip("ZR")
...
def parse_indexes(text: str = "", seq: bool = False):
    ...
    indexes_pattern = r"\s+(?:(?P<indexes>\d{2,3}(?:/\d{2,3})?\s?(?:[A-ZТ]|ZR)))"
...
def parse_stud(text: str = "", seq: bool = False):
    ...
    # stud_pattern = r"\s+(?P<stud>ШИПЫ|^ШИПЫ|ШИП|^ШИП|Ш|^Ш|STUD|^STUD\b)"
